...
/**
 * Postgresql용 DDL Mapper
 * 
 * @author shortstop
 */
public class DdlMapperPostgresql implements DdlMapper {
	...
	public String tableTemplate() {
		...
		joiner.add(")");
		joiner.add("WITH (OIDS=FALSE);");
		joiner.add("ALTER TABLE $tableName OWNER TO $account;");
		return joiner.toString();
	}
	...
	public String indexTemplate() {
		...
		joiner.add("		CREATE UNIQUE INDEX $index.name ON $tableName USING btree ($index.columnList);");
		joiner.add("	#else");
		joiner.add("		CREATE INDEX $index.name ON $tableName USING btree ($index.columnList);");
		joiner.add("	#end");
		joiner.add("\n");
		joiner.add("#end");
		return joiner.toString();
	...
	public String dropProcedureTemplate(String name) {
		...
		sql.add("#if($paramTypes)");
		sql.add("DROP FUNCTION " + name + "(${paramTypes})");
		sql.add("#else");
		sql.add("DROP FUNCTION " + name + "()");
		sql.add("#end");
		return sql.toString();
	}
	...
	public String getPprocedureDefTemplate() {
		StringJoiner sql = new StringJoiner("\n");
		sql.add("SELECT PG_GET_FUNCTIONDEF(oid)");
		sql.add("FROM PG_PROC WHERE PRONAME = '${name}'");
		return sql.toString();
	}
}