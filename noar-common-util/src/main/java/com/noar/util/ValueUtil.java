...
public class ValueUtil {
	...
	public static String getValueByFilePath(String path) {
		return getValueByFilePath(path, "UTF-8");

	}

	public static String getValueByFilePath(String path, String charSet) {
		if (ValueUtil.isEmpty(path)) {
			return null;
		}

		try {
			return IOUtils.toString(new ClassPathResource(path.trim()).getInputStream(), charSet);
		} catch (IOException e) {
